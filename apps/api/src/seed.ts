...
/** Popola il DB con i dati demo (classe, maestra, bambini, esercizi, tentativi storici). */
export function seed(): void {
  ...
  [0.5, 0.48, 0.55, 0.52].forEach((acc, i) => {
    attempt({ child: 'child-fatima', ex: 'letter-A', acc, dur: 110000, stalls: 4, hints: 6, oob: 18, day: 5 - i });
  });
  attempt({
    child: 'child-fatima',
    ex: 'letter-A',
...
/** Esegue il seed solo se il DB è vuoto (usato all'avvio del server). */
export function seedIfEmpty(): boolean {
  initSchema();
  const { n } = db.prepare('SELECT COUNT(*) AS n FROM teachers').get() as { n: number };
  if (n === 0) {
    seed();
    return true;
  }
...
// Esecuzione da CLI: `tsx src/seed.ts`
const isDirectRun =
